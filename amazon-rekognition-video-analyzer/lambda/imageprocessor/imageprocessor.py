...
def process_image(event, context):
    ...
    rekog_min_conf = config["rekog_min_conf"]
         
    label_watch_list = config["label_watch_list"]
    label_watch_min_conf = config["label_watch_min_conf"]
    
   # label_watch_min_conf=50.0
    ...
    for record in event['Records']:
        ...
        for label in rekog_response['Labels']:
            ...
            print("datatype",type(conf))
            print("Intance=",label['Instances'],"datatype",type(label['Instances']))
            label['Instances']=[]
